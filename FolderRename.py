...
# --- Cấu hình giao diện ---
st.set_page_config(page_title="Character Drive Cloner", layout="wide")
st.title("📂 Character Folder Cloner")

# Truy cập service đã khởi tạo trong drive_ops
service = drive_ops.drive_service

def clone_folder_logic(source_id, target_parent_id, new_name):
    """
    ...
    for item in items:
        ...
        if item['mimeType'] == 'application/vnd.google-apps.folder':
            clone_folder_logic(item['id'], new_id, item['name'])
...
if working_id:
    ...
    try:
        ...
        if not templates:
            ...
            # Sidebar Selector
            selected_tpl = st.sidebar.selectbox("🎯 Chọn Folder Template:", template_names)
            ...
            with col1:
                num_to_create = st.number_input("Số lượng folder clone:", min_value=1, max_value=5, value=1)
            ...
            if st.button(f"🚀 Xác nhận tạo {num_to_create} bản sao cho '{selected_tpl}'"):
                ...
                with st.status("Đang thực hiện sao chép...", expanded=True) as status:
                    for i in range(num_to_create):
                        ...
                        clone_folder_logic(source_id, working_id, new_name)
